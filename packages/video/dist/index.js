...
function renderMainFragment ( root, component ) {
	...
	div.style.cssText = "width: " + ( root.width ) + "px; height: " + ( root.height ) + "px;";
	
	var video = document.createElement( 'video' );
	component.refs.video = video;
	video.width = root.width;
	video.height = root.height;
	video.src = root.src;
	video.loop = root.loop;
	
	div.appendChild( video );
	div.appendChild( document.createTextNode( "\n  " ) );
	
	var div1 = document.createElement( 'div' );
	div1.className = "sf-video-controls";
	
	div.appendChild( div1 );
	
	var label = document.createElement( 'label' );
	label.className = "sf-video-controls--play";
